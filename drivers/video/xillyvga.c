...
static int __devinit xillyvga_of_probe(struct platform_device *op)
{
	struct xillyvga_platform_data pdata;
	struct resource res;
	int rc;
...
static int __devexit xillyvga_of_remove(struct platform_device *op)
{
	return xillyvga_release(&op->dev);
}

/* Match table for of_platform binding */
static struct of_device_id xillyvga_of_match[] __devinitdata = {
...
static struct platform_driver xillyvga_of_driver = {
	...
	.remove = __devexit_p(xillyvga_of_remove),
	.driver = {
		.name = DRIVER_NAME,
		.owner = THIS_MODULE,
